...
get_header(); 
?>
<section class="sec-evnt-pagi position-relative">
	<div class="regular-container display-flx">
		<div class="w-393 b-info">
      <?php if(get_field('content_page_heading','option')){ ?>
         <h1 class="event-h1"> <?php the_field('content_page_heading','option'); ?></h1>
         <?php } ?>
         <?php if(get_field('contents_page_description','option')){ ?>
         <p class="b-info"><?php the_field('contents_page_description','option'); ?></p>
         <?php } ?>
		</div>
	</div>
</section>
<section class="content-soft">
	<div class="regular-container">
		...
		<form class="chek-select">
			<div class="filter-check">
				<div class="dopdown-checkbox-1">
					<select id="rating_star" name="" multiple="multiple" class="startfilters" onchange="content_filter()">
						<option value="5">Show 5 stars content</option>
						<option value="4">Show 4 stars content</option>
						<option value="3">Show 3 stars content</option>
						<option value="2">Show 2 stars content</option>
						<option value="1">Show 1 stars content</option>
					</select>
				</div>
				<div class="dopdown-checkbox-2 ">
					<select id="prod_assgn" name="" multiple="multiple" class="chooseHfilter" onchange="content_filter()">
            <?php $con_args = array(
                    'post_type' => 'contents',
                    'posts_per_page' => -1,
                    'post_status' => 'publish'
                  );
                  $the_main_loop = new WP_Query( $con_args ); 
                  if($the_main_loop->have_posts()) { 
                    $sel_prod_arr = array();
                    while($the_main_loop->have_posts()) { 
                    $the_main_loop->the_post(); 
                    $current_id = get_the_ID();
                    $assigned_prod = get_field('product_assign',$current_id); 
                      foreach( $assigned_prod as $sel_assigned_prod ){
                     
                        array_push($sel_prod_arr,$sel_assigned_prod);
                      }
                    }
                    $sel_prod_arr = array_unique($sel_prod_arr);
                    wp_reset_query();
                    wp_reset_postdata();
                  }
                 echo '<option value="all" class="all-headset">All headsets</option>';
                  foreach($sel_prod_arr as $option_prod){
						        echo '<option value="'.$option_prod.'">'.get_the_title($option_prod).'</option>';
                  } ?>
					</select>
				</div>
				<div class="dopdown-checkbox-2">
					<select  id="contentterm" multiple="multiple" class="Genresfilter" name="term_name" onchange="content_filter()">
					<?php
            ...
            foreach ($terms as $term_cat) { 
              ...
              echo '<div class="action-horror mb-38">
                        ...
                        <div class="index-product slider">
                             <div id="filterData">
                                <h2 class="event-title-1">'. $term_name.'</h2> 	
     ...
     </div> <script>carasouel("Actionproducts'.$term_cat_id.'")</script>';
			      } ?>
		</div>
		</div>
		<!-- type 1 soft -->
